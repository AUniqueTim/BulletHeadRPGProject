...
public class WorldMapPlayerMovement : MonoBehaviour
{
    ...
    public static WorldMapPlayerMovement instance;
    public static WorldMapPlayerMovement Instance
    {
        get
        {
            if (instance == null)
            {
                instance = GameObject.FindObjectOfType<WorldMapPlayerMovement>();
                if (instance == null)
                {
                    GameObject singleton = new GameObject();
                    singleton.AddComponent<WorldMapPlayerMovement>();
                    singleton.name = "(Singleton) WorldMapPlayerMovement";
                }
            }
            return instance;
        }
    ...
    public void RunStates()
    {
        ...
        if (Input.GetKey(KeyCode.R)) { MoveForward(); gameObject.transform.Translate(Vector3.forward * playerSpeed * Time.deltaTime); runState = "posZAxis"; }
        if (Input.GetKey(KeyCode.F)) { MoveForward(); gameObject.transform.Translate(Vector3.back * playerSpeed * Time.deltaTime); runState = "negZAxis"; }

        if (Input.GetKey(KeyCode.W)) { MoveForward(); gameObject.transform.Translate(Vector3.up * playerSpeed * Time.deltaTime); runState = "posYAxis"; gravity = 0; }
        else { gravity = -9.87f; }
        if (Input.GetKey(KeyCode.S)) { MoveBackward(); gameObject.transform.Translate(Vector3.down * playerSpeed * Time.deltaTime); runState = "negYAxis"; gravity = 0; }
        else { gravity = -9.87f; }
